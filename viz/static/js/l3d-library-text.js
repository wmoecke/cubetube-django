...
class Text {
	...
	int selectedFont;
	 ...
	 void showChar(char a, PVector p, int col)
	 {
	 ...
	 void showChar(char a, PVector origin, PVector angle, int col)
	 {
		 showChar(a, origin, new PVector(0,0,0), angle, col);
	 }

	 void showChar(char a, PVector origin, PVector pivot, PVector angle, int col)
	 {
	 ...
	 void scrollSpinningText(String text, PVector initialPosition, int col)
	 {
	 ...
	 void scrollText(String text, PVector initialPosition, int col)
	 {
		 for(int i=0;i<text.length();i++)
			 showChar(text.charAt(i), new PVector(8*i-initialPosition.x, initialPosition.y, initialPosition.z), col);
	 }
	 
	 void marquee(String text, float pos, int col)
	 {
		 marquis(text, pos, col);  //fixed a spelling error
	 }
	 
	 void marquis(String text, float pos, int col)
	 {
	 ...
	 void showMarqueeChar(char a, int pos, int col)
	 {
		 showMarquisChar(a,pos,col);  //fixed a spelling error
	 }

	 void showMarquisChar(char a, int pos, int col)
	 {		 
		 for(int row=0;row<8;row++)
			 for(int bit=0;bit<8;bit++)
				 if(((font[((int)a+selectedFont)*8+row]>>(7-bit))&0x01)==1)
